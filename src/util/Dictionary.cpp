...
uint32_t Dictionary::bisectSamples(HalFile& sidecar, HalFile& source, uint32_t sampleCount, const char* target) {
  ...
  while (lo < hi) {
    ...
    // The linear scan below normalizes every headword it reads; the bisect must use the same
    // key or it descends on "Intelligenz," while the scan looks for "Intelligenz" and lands on
    // the wrong offset (Babylon's Duden conversion, see normalizeIndexHeadword).
