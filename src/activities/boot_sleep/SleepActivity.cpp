...
void SleepActivity::renderTransparentCustomSleepScreen() const {
  ...
  // Nothing in the upstream locations: fall back to the fork's own, so a card set up before
  // this merge still works. Only after both, because the upstream BGRA path is the better
  // renderer (dithered partial alpha, grayscale planes) and should win when both exist.
  if (renderLegacyTransparentOverlay()) return;

  LOG_ERR("SLP", "No valid transparent sleep overlay found");
  renderDefaultSleepScreen();
}

// The fork's pre-merge overlay locations. BMPs here go through the same renderer as the
// upstream ones; PNGs take the alpha-mask decoder below, which upstream has no equivalent for.
bool SleepActivity::renderLegacyTransparentOverlay() const {
  static constexpr const char* kLegacyDirs[] = {"/.sleep/transparent", "/sleep/transparent"};
  for (const char* dirPath : kLegacyDirs) {
    auto dir = Storage.open(dirPath);
    if (!dir || !dir.isDirectory()) continue;

    // Collect names first: the random pick needs the count, and re-walking the directory per
    // candidate costs an SD scan each time.
    std::vector<std::string> files;
    files.reserve(16);
    // Heap, not stack: MAX_SLEEP_FILE_NAME_LEN is 256 bytes and the stack budget for a local
    // is under that (same reason selectRandomSleepFile above allocates it).
    auto name = makeUniqueNoThrow<char[]>(MAX_SLEEP_FILE_NAME_LEN);
    if (!name) {
      LOG_ERR("SLP", "OOM: legacy overlay filename buffer");
      return false;
    }
    for (auto dirFile = dir.openNextFile(); dirFile; dirFile = dir.openNextFile()) {
      if (dirFile.isDirectory()) continue;
      ...
      if (name[0] == '\0' || name[0] == '.') continue;
      const std::string_view entryName{name.get()};
      if (FsHelpers::hasBmpExtension(entryName) || FsHelpers::hasPngExtension(entryName)) {
    ...
    const auto index = static_cast<size_t>(random(static_cast<long>(files.size())));
    const std::string path = std::string(dirPath) + "/" + files[index];
    LOG_DBG("SLP", "Legacy transparent overlay: %s", path.c_str());

    if (FsHelpers::hasPngExtension(path)) {
      if (renderPngOverlaySleepImage(path)) return true;
      continue;
    }
    HalFile file;
    if (Storage.openFileForRead("SLP", path, file) && renderSleepOverlayFile(file, path.c_str())) return true;
  }
  return false;
}

// PNG overlays render through the shared PNG->framebuffer decoder with alphaMask set:
// alpha < 128 keeps the book page, everything else -- including white -- covers it. This is a
// hard threshold, not upstream's Bayer dither, because the decoder composites per pixel without
// the screen coordinates the dither needs.
bool SleepActivity::renderPngOverlaySleepImage(const std::string& path) const {
  ...
  // overlay silently falls back to the plain screen. Release the font caches first: the
  // retained framebuffer (the page the overlay shows through) is a separate buffer and is
  // unaffected, and fonts reload on wake.
  ...
  if (scale > 1.0f) scale = 1.0f;
  const int dstWidth = static_cast<int>(static_cast<float>(dims.width) * scale);
  const int dstHeight = static_cast<int>(static_cast<float>(dims.height) * scale);
