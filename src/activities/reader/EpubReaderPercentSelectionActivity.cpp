...
void EpubReaderPercentSelectionActivity::loop() {
  ...
  // Step size follows the physical PAIR, not a screen axis. The hints promise "front buttons step
  // small, side buttons step large", and the rotation swaps which axis each pair serves -- naming
  // ScreenLeft/Right for the small step put the LARGE step on the front buttons in landscape while
  // the hints still claimed the opposite.
  const auto smallDown = mappedInput.frontPairPrevious();
  const auto smallUp = mappedInput.frontPairNext();
  const bool frontIsVertical = smallDown == MappedInputManager::Button::ScreenUp;
  // The side pair is whichever axis the front pair is not currently serving.
  const auto largeUp = frontIsVertical ? MappedInputManager::Button::ScreenLeft : MappedInputManager::Button::ScreenUp;
  const auto largeDown =
      frontIsVertical ? MappedInputManager::Button::ScreenRight : MappedInputManager::Button::ScreenDown;

  buttonNavigator.onPressAndContinuous({smallDown}, [this] { adjustPercent(-kSmallStep); });
  buttonNavigator.onPressAndContinuous({smallUp}, [this] { adjustPercent(kSmallStep); });
  ...
  // This is about where the side buttons sit on the CASE, so it stays keyed to the board, not to the
  // rotation -- the two corrections are independent.
  ...
  buttonNavigator.onPressAndContinuous({largeUp}, [this, upDelta] { adjustPercent(upDelta); });
  buttonNavigator.onPressAndContinuous({largeDown}, [this, downDelta] { adjustPercent(downDelta); });
}
