...
- `--no-panels` — skip panel detection entirely: every page becomes a single full-page "panel" with no crop files, so the reader behaves as a pure page flipper (page-turn buttons always turn pages, never enter panel-zoom). Use for image-based novels, photo books, or scans where panel boxes are meaningless — the detector would otherwise carve text columns into bogus panels. Implies `--no-ocr` (full-page panels are never OCR'd, so no Gemini key is needed)
...
- `--x3` / `--x4` — downscale pages **and** panel crops to fit the device screen: `--x3` targets the Xteink X3 (528×792), `--x4` the Xteink X4 (480×800); landscape images fit the rotated screen box, matching how the reader rotates them to fill the display. The device can never show more pixels than its screen, so this loses nothing visually while making files smaller and on-device decoding (page turns, panel entry) noticeably faster — high-resolution scans shrink to a fraction of their size. Never upscales; works with every format (JPEG/PNG/BMP, with or without `--mono`). Without either flag the original resolution is kept.
