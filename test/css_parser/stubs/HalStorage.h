...
class HalFile {
 public:
  ...
  // Absolute seek: the incremental writer patches the rule count back into the header.
...
class HalStorage {
 public:
  ...
  bool openFileForWrite(const char*, const char* path, HalFile& file) { return file.open(path, "w+b"); }
