...
class HalFile {
 public:
  ...
  // Bytes left from the cursor. This fork's readString() uses it to reject a length that runs
  // past the end of the file, so the stub has to answer it too.
  int available() const {
    if (!node) return 0;
    return pos >= node->bytes.size() ? 0 : static_cast<int>(node->bytes.size() - pos);
  }
