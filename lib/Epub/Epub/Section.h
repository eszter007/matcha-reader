...
class Section {
 ...
 public:
  ...
  // Read a page straight from the committed file with a LOCAL handle -- no currentPage, no
  // member file handle, no build state. That is what makes it safe from the render task's
  // background image warm while the loop task owns the build. Returns nullptr if the
  // committed file doesn't have that page yet.
