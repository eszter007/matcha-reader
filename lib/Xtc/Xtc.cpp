...
bool Xtc::generateThumbBmp(int height, CancelFn shouldCancel, void* cancelCtx) const {
  ...
  // Page and row buffers in one block: two separate allocations of this size fragment the
  // heap the image decoders need next.
  auto thumbScratch = makeUniqueNoThrow<uint8_t[]>(bitmapSize + rowSize);
  if (!thumbScratch) {
    // ~104KB for a 2-bit 528x792 page, which does not fit the largest contiguous block on a
    // fragmented heap (e.g. the Home screen). For 2-bit pages fall back to a low-memory
    // streaming generator that reads the cover column-by-column (~5KB). 1-bit pages are half
    // the size and keep the simple path.
    if (bitDepth == 2) {
      LOG_INF("XTC", "Thumb buffers (%lu bytes) OOM; using streaming thumb generator",
